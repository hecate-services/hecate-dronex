...
c(Xm, Ym, Tick) -> sensor:contact(one, Tick, Xm * ?M, Ym * ?M, 100 * ?M, 900).

%% Feed the same position for N consecutive ticks.
held(Xm, Ym, Ticks) ->
    lists:foldl(fun (T, S) -> tracks:advance(S, [c(Xm, Ym, T)], T) end,
                tracks:empty(), lists:seq(1, Ticks)).
...
one_contact_is_a_track_but_not_a_confirmed_one_test() ->
    ...
    ?assertEqual([], tracks:confirmed(S)).

enough_evidence_confirms_it_test() ->
    ?assertEqual([], tracks:confirmed(held(100, 100, ?CONFIRM_EVIDENCE - 1))),
    ?assertEqual(1, length(tracks:confirmed(held(100, 100, ?CONFIRM_EVIDENCE)))).
...
a_lone_ghost_never_confirms_test() ->
    ...
    S = tracks:advance(tracks:empty(), [c(300, 300, 1)], 1),
    Aged = lists:foldl(fun (T, A) -> tracks:advance(A, [], T) end, S,
                       lists:seq(2, ?TRACK_DROP_TICKS + 2)),
    ?assertEqual([], tracks:confirmed(Aged)),
    ?assertEqual(0, tracks:count(Aged)).
...
contacts_far_apart_are_different_targets_test() ->
    S = tracks:advance(tracks:empty(), [c(100, 100, 1), c(600, 600, 1)], 1),
    ?assertEqual(2, tracks:count(S)).

contacts_inside_the_gate_are_the_same_target_test() ->
    Near = (?TRACK_GATE div ?M) div 2,
    S = tracks:advance(tracks:empty(), [c(100, 100, 1)], 1),
    S2 = tracks:advance(S, [c(100 + Near, 100, 2)], 2),
    ?assertEqual(1, tracks:count(S2)).
...
two_targets_closer_than_the_gate_merge_test() ->
    ...
    S = tracks:advance(tracks:empty(), [c(100, 100, 1), c(100 + Tight, 100, 1)], 1),
...
a_fast_mover_stays_one_track_test() ->
    ...
    Step = 20,
    S = lists:foldl(fun (T, A) -> tracks:advance(A, [c(100 + T * Step, 100, T)], T) end,
                    tracks:empty(), lists:seq(1, 12)),
    ...
    %% worth seeing.
    ?assertEqual(1, tracks:count(S)),
    ?assertEqual(1, length(tracks:confirmed(S))).

a_track_carries_a_velocity_test() ->
    S = lists:foldl(fun (T, A) -> tracks:advance(A, [c(100 + T * 20, 100, T)], T) end,
                    tracks:empty(), lists:seq(1, 12)),
    [#{vx := Vx, vy := Vy}] = tracks:confirmed(S),
    ?assert(Vx > 0),
    ?assertEqual(0, Vy).

the_position_is_smoothed_not_replaced_test() ->
    S = tracks:advance(tracks:empty(), [c(100, 100, 1)], 1),
    S2 = tracks:advance(S, [c(120, 100, 2)], 2),
    [#{x := X}] = S2,
...
it_survives_a_gap_shorter_than_the_drop_test() ->
    ...
    Aged = lists:foldl(fun (T, A) -> tracks:advance(A, [], T) end, S,
                       lists:seq(Last + 1, Last + ?TRACK_DROP_TICKS - 1)),
    ?assertEqual(1, tracks:count(Aged)),
...
a_contact_arriving_on_the_drop_tick_saves_the_track_test() ->
    ...
    ?assertEqual(1, tracks:count(Saved)).
...
status_is_bit_flags_on_an_integer_test() ->
    ...
    [C] = tracks:confirmed(held(100, 100, ?CONFIRM_EVIDENCE)),
